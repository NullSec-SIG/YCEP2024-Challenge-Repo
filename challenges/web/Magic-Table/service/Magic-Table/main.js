...
function htmlElement(table, i) {
    ...
    element.addEventListener('click', () => elementClickHandler(i, element), false);
    return element;
}

function elementClickHandler(i, e) {
    e.classList.toggle("highlighted");
    const isHighlighted = e.classList.contains("highlighted");
    e.style.border = isHighlighted ? "2px solid rgba(0,127,127,1)" : "";

    const url = new URL(window.location.href);
    const params = new URLSearchParams(url.search);

    if (isHighlighted) {
        const paramName = `element${params.size + 1}`;
        params.set(paramName, i);
    } else {
    ...
    for (const [_, value] of params.entries()) {
      const newKey = `element${newParams.size + 1}`;
...
function updateStatus() {
    ...
    const requestUrl = `api.php?${urlParams.toString()}`;
